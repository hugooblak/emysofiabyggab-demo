...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'emysofiabyggab-leads' ),
					'singular_name' => __( 'Lead', 'emysofiabyggab-leads' ),
					'edit_item'     => __( 'Förfrågan', 'emysofiabyggab-leads' ),
					'search_items'  => __( 'Search leads', 'emysofiabyggab-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'emysofiabyggab-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'emysofiabyggab-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'emysofiabyggab-leads' ),
			'npl_phone'    => __( 'Phone', 'emysofiabyggab-leads' ),
			'npl_service'  => __( 'Needs', 'emysofiabyggab-leads' ),
			'npl_timeline' => __( 'When', 'emysofiabyggab-leads' ),
			'npl_zip'      => __( 'ZIP', 'emysofiabyggab-leads' ),
			'npl_source'   => __( 'Source', 'emysofiabyggab-leads' ),
			'npl_status'   => __( 'Status', 'emysofiabyggab-leads' ),
			'date'         => __( 'Received', 'emysofiabyggab-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'emysofiabyggab-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'emysofiabyggab-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'emysofiabyggab-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'emysofiabyggab-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'emysofiabyggab-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'emysofiabyggab-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'emysofiabyggab-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'emysofiabyggab-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'emysofiabyggab-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'emysofiabyggab-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'emysofiabyggab-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'emysofiabyggab-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
