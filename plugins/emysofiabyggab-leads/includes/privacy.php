...
add_filter(
	...
	function ( $exporters ) {
		$exporters['emysofiabyggab-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'emysofiabyggab-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'emysofiabyggab-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'emysofiabyggab-leads',
						'group_label' => __( 'Quote requests', 'emysofiabyggab-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['emysofiabyggab-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'emysofiabyggab-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
