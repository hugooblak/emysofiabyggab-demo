<?php
/**
 * EmySofiaByggAB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "EmySofiaByggAB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through em_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'emysofiabyggab', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'emysofiabyggab' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'emysofiabyggab' ) ) );

		register_block_pattern_category( 'emysofiabyggab-sections', array( 'label' => __( 'EmySofiaByggAB sections', 'emysofiabyggab' ) ) );
		register_block_pattern_category( 'emysofiabyggab-pages', array( 'label' => __( 'EmySofiaByggAB page layouts', 'emysofiabyggab' ) ) );
	}
);
