 ...
 * or photo. They call em_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional EmySofiaByggAB.
 *
 ...
 * @return string
 */
function em_sample_note() {
	return em_lead( 'ar_riktig' ) ? '' : ' <span class="em-sample">(platshållare)</span>';
}
 ...
 * @return string File name for em_lead_img().
 */
function em_lead_bild( $i ) {
	$bilder = (array) em_lead( 'bilder', array() );
 ...
 * @return string
 */
function em_ring_label() {
	$namn = em_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, em_lead( 'telefon' ) ) : 'Ring ' . em_lead( 'telefon' );
}
